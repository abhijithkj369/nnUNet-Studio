    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                # Handle small datasets where n_samples < n_splits
                n_splits = 5
                if len(all_keys_sorted) < n_splits:
                    n_splits = len(all_keys_sorted)
                    self.print_to_log_file(f"WARNING: Dataset has fewer than 5 samples ({len(all_keys_sorted)}). "
                                           f"Reducing n_splits to {n_splits}.")
                
                if n_splits > 1:
                    splits = generate_crossval_split(all_keys_sorted, seed=12345, n_splits=n_splits)
                else:
                    # Fallback for single sample or very small datasets
                    splits = []
                    for i in range(5): # Create 5 dummy folds
                        splits.append({'train': all_keys_sorted, 'val': all_keys_sorted})
